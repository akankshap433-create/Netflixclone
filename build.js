...
function copyRecursiveSync(src, dest) { var exists = fs.existsSync(src); var stats = exists && fs.statSync(src); var isDirectory = exists && stats.isDirectory(); if (isDirectory) { if (!fs.existsSync(dest)) { fs.mkdirSync(dest, {recursive: true}); } fs.readdirSync(src).forEach(function(childItemName) { copyRecursiveSync(path.join(src, childItemName), path.join(dest, childItemName)); }); } else { fs.copyFileSync(src, dest); } }
copyRecursiveSync('assets', 'dist/assets');
copyRecursiveSync('src/styles', 'dist/src/styles');
let indexHtml = fs.readFileSync('index.html', 'utf8'); indexHtml = indexHtml.replace(/dist\//g, ''); fs.writeFileSync('dist/index.html', indexHtml, 'utf8');
console.log('Copied assets, styles, and index.html to dist/ for static deployment.');
